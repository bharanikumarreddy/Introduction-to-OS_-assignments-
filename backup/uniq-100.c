...
int str_cmp(const char *str1, const char *str2) {
    for (int i = 0; str1[i] && str2[i]; ++i) {
        ...
        char char2 = str2[i] | 32; // Convert to lowercase
        if (char1 != char2) {
            return char1 - char2;
        }
    }
...
int final_count = 0;
int k, x, y, z, current_index = 0, m = 0, line_count = 0, b, bytes_read, i, char_count = 0, duplicate_count = 0;
...
int main(int argc, char **argv) {
    ...
    if (argc <= 1) {
        unique_lines(0, "", 0, 0, 0); // Default behavior: no options
        exit();
    } else if (argc == 2) {
        for (r = 1; r < argc; r++) {
            ...
            unique_lines(file_descriptor, argv[r], 0, 0, 0); // Default behavior: no options
        ...
        for (r = 1; r < argc; r++) {
            if (strcmp(argv[r], "-c") == 0 || strcmp(argv[r], "-C") == 0) count_lines = 1;
            else if (strcmp(argv[r], "-d") == 0 || strcmp(argv[r], "-D") == 0) print_duplicates = 1;
            else if (strcmp(argv[r], "-i") == 0 || strcmp(argv[r], "-I") == 0) ignore_case = 1;
            else break;
        }
